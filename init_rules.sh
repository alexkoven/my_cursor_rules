...
# Check if we're not in /Code/my_cursor_rules/
current_dir=$(realpath "$(pwd)")
if [[ "$current_dir" == "/home/falcon/Code/my_cursor_rules" ]]; then
    echo "Error: This script must not be run from /Code/my_cursor_rules/"
    exit 1
fi
